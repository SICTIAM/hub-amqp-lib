...
import fr.sictiam.amqp.api.basic.AmqpBasicServer
...
class AmqpBasicServerSpec extends AmqpSpec {
  val headers = Map.empty[String, JsValue]

  override implicit val patienceConfig = PatienceConfig(10.seconds)

  "The AmqpBasicServer" should {
    val queueName: String = s"basic-server-test-queue-" + System.currentTimeMillis()

    val server = new AmqpBasicServer(queueName, "serverTest")
    val messages = Vector(
