...
class AmqpConnectorsSpec extends AmqpSpec {
  ...
  "The AMQP Connectors" should {
    ...
    val connectionProvider = AmqpDetailsConnectionProvider("invalid", 5673)
      .withCredentials(AmqpCredentials(AmqpClientConfiguration.user, AmqpClientConfiguration.pwd))
      .withHostsAndPorts(immutable.Seq(AmqpClientConfiguration.host -> AmqpClientConfiguration.port))
    ...
    "keep connection open if downstream closes and there are pending acks" in {
      val connectionSettings = AmqpDetailsConnectionProvider(AmqpClientConfiguration.host, 5672)
        .withCredentials(AmqpCredentials(AmqpClientConfiguration.user, AmqpClientConfiguration.pwd))
    ...
    "declare connection that does not require server acks" in {
      val connectionProvider = AmqpDetailsConnectionProvider(AmqpClientConfiguration.host, 5672)
        .withCredentials(AmqpCredentials(AmqpClientConfiguration.user, AmqpClientConfiguration.pwd))
